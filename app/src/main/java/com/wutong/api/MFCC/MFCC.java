package com.wutong.api.MFCC;

/**
 * Created by TongWu on 4/15/18.
 */
...
public class MFCC {
    ...
    private double[][] stftMagSpec(double[] y){
        ...
        for (int j = 0; j < y.length; j++) ypad[(n_fft / 2) + j] = y[j];
    ...
    private double[][] dctFilter(int n_filters, int n_input){
        ...
        for (int j = 0; j < n_input; j++) basis[0][j] = 1.0 / Math.sqrt(n_input);
